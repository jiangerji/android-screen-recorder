...
/**
 * Activity for the actual game. This is largely just a wrapper for our
 * GLSurfaceView.
 */
public class GameActivity extends Activity {
    ...
    protected void onPause() {
        /*
         * We must call the GLView's onPause() function when the framework tells
         * us to pause.
         * We're also expected to deallocate any large OpenGL resources, though
         * presumably
         * that just means our associated Bitmaps and FloatBuffers since the
         * OpenGL goodies
         * themselves (e.g. programs) are discarded by the GLSurfaceView.
         * 
         * Our GLSurfaceView's onPause() method will synchronously invoke the
         * GameState's save()
         * function on the Renderer thread. This will record the saved game into
         * the storage
         ...
         * 
         * This isn't the ideal place to do this, because if the devices loses
         * power while
         * sitting on the "game over" screen we won't record the score. In
         * practice the
         * user will either leave the game or the device will go to sleep,
         * pausing the activity,
         * so it's not a real concern.
         * 
         * We could improve on this by having GameState manage the high score,
         * but since
         * we're using Preferences to hold it, we'd need to pass the Activity
         * through. This
         * interferes with the idea of keeping GameState isolated from the
         * application UI.
         * 
         * Note that doing this update in the Preferences code in
         * BreakoutActivity is a
         * bad idea, because that would prevent us from recording a high score
         * until the user
         * hit "back" to return to the initial Activity -- which won't happen if
         * they just
         * hit the "home" button to quit.
         * 
         * BreakoutActivity will need to see the updated high score. The Android
         * lifecycle
         * is defined such that our onPause() will execute before
         * BreakoutActivity's onResume()
         * is called (see "Coordinating Activities" in the developer guide page
         * for Activities),
         * so they'll be able to pick up whatever we do here.
         * 
         * We need to do this *after* the call to mGLView.onPause(), because
         * that causes
         * GameState to save the game to static storage, and that's what we read
         * the score from.
         */
    ...
    protected void onResume() {
        /*
         * Complement of onPause(). We're required to call the GLView's
         * onResume().
         * 
         * We don't restore the saved game state here, because we want to wait
         * until after the
         * objects have been created (since much of the game state is held
         * within the objects).
         * In any event we need it to run on the Renderer thread, so we let the
         * restore happen
    ...
    /**
     * Configures the GameState object with the configuration options set by
     * BreakoutActivity.
     */
    private void configureGameState() {
        ...
        case 3: // absurd
            ...
            break;
        default:
            throw new RuntimeException("bad difficulty index "
                    + sDifficultyIndex);
        }
    ...
    /**
     * Gets the default difficulty index. This should be used if no preference
     * has been saved.
     */
    ...
    public static void setDifficultyIndex(int difficultyIndex) {
        ...
        if (difficultyIndex < DIFFICULTY_MIN
                || difficultyIndex > DIFFICULTY_MAX) {
            Log.w(TAG, "Invalid difficulty index " + difficultyIndex
                    + ", using default");
    ...
    /**
     * Configures the "never lose a ball" option. If set, the ball bounces off
     * the bottom
    ...
    /**
     * Updates high score. If the new score is higher than the previous score,
     * the entry
     * is updated.
     * 
     * @param lastScore
     *            Score from the last completed game.
     */
    private void updateHighScore(int lastScore) {
        SharedPreferences prefs = getSharedPreferences(BreakoutActivity.PREFS_NAME,
                MODE_PRIVATE);
