 ...
 * <p>
 * The methods here expect to run on the Renderer thread. Calling them from
 * other threads must be done through GLSurfaceView.queueEvent().
 */
public class GameSurfaceRenderer implements GLSurfaceView.Renderer {
    ...
    /**
     * Constructs the Renderer. We need references to the GameState, so we can
     * tell it to
     * update and draw things, and to the SurfaceView, so we can tell it to stop
     * animating
     * when the game is over.
     */
    public GameSurfaceRenderer(GameState gameState,
    ...
    /**
     * Handles initialization when the surface is created. This generally
     * happens when the
     * activity is started or resumed. In particular, this is called whenever
     * the device
     * is rotated.
     * <p>
    ...
    /**
     * Performs one-time GL setup (creating programs, disabling optional
     * features).
     */
    ...
    /**
     * Updates the configuration when the underlying surface changes. Happens at
     * least once
     * after every onSurfaceCreated().
     * <p>
     * If we visit the home screen and immediately return, onSurfaceCreated()
     * may not be called, but this method will.
     */
    @Override
    public void onSurfaceChanged(GL10 unused, int width, int height) {
        /*
         * We want the viewport to be proportional to the arena size. That way a
         * 10x10
         * object in arena coordinates will look square on the screen, and our
         * round ball
         * will look round.
         * 
         * If we wanted to fill the entire screen with our game, we would want
         * to adjust the
         * size of the arena itself, not just stretch it to fit the boundaries.
         * This can have
         * subtle effects on gameplay, e.g. the time it takes the ball to travel
         * from the top
         * to the bottom of the screen will be different on a device with a 16:9
         * display than on
         * a 4:3 display. Other games might address this differently, e.g. a
         * side-scroller
         * could display a bit more of the level on the left and right.
         * 
         * We do want to fill as much space as we can, so we should either be
         * pressed up against
         * the left/right edges or top/bottom.
         * 
         * Our game plays best in portrait mode. We could force the app to run
         * in portrait
         * mode (by setting a value in AndroidManifest, or by setting the
         * projection to rotate
         * the world to match the longest screen dimension), but that's
         * annoying, especially
        ...
        Log.d(TAG, " --> x=" + x + " y=" + y + " gw=" + viewWidth + " gh="
                + viewHeight);
    ...
    private void saveRenderState() {
        System.arraycopy(mProjectionMatrix,
                0,
                mSavedMatrix,
                0,
    ...
    public void onDrawFrame(GL10 unused) {
        ...
        GLES20.glViewport(mViewportXoff,
                mViewportYoff,
                mViewportWidth,
                mViewportHeight);
        drawFrame();
        ...
        if (true) {
            saveRenderState();
            GameActivity.mCapturing.beginCaptureFrame();
            //            GameActivity.mCapturing.getProjectionMatrix(mProjectionMatrix);
            //            GameActivity.mCapturing.setViewport();
            drawFrame();
            GameActivity.mCapturing.endCaptureFrame();
            restoreRenderState();
        }
    ...
    private void drawFrame() {
         ...
         * 
         * We have to know if the images use pre-multiplied alpha or not so we
         * can choose the
         * appropriate blend mode. The linear interpolation performed by our
         * fragment shader
         * is going to interpolate across all four RGBA color components if we
         * use GL_LINEAR
         * for our texture filter, which means the anti-aliased edges will
         * effectively be
         * premultiplied. (An interpolated transition from opaque white
         * 0xffffffff to
         * transparent black 0x00000000 might be 0x7f7f7f7f; we want that to be
         * 50% transparent
         * white, not 50% transparent medium-gray.) If we blend with
         * (GL_SRC_ALPHA,
         * GL_ONE_MINUS_SRC_ALPHA), we will see grey edges on the white ball,
         * very noticeable on
         * lighter backgrounds. We want to use GL_ONE for the first parameter
         * instead (a/k/a
         * "1, 1-src").
         * 
         * The ball texture itself uses only two colors, transparent black and
         * opaque white, so
         * we could avoid the edge artifacts we see with GL_SRC_ALPHA by
         * filtering with
         * GL_NEAREST instead, since that keeps us from sampling multiple texels
         * to determine
         * the fragment color. It's generally easier to work with premultiplied
         * alpha though,
         * and we really want to use GL_LINEAR to avoid chunkiness when the ball
         * is scaled.
         * 
         * (If we rendered the score digit texture data on top of the background
         * color,
         * rather than transparent black, we wouldn't need to alpha-blend it
         * here since we
         * know it's never near anything but the ball. Drawing without blending
         * would likely
         * be faster.)
         */
        ...
        GLES20.glBlendFunc(GLES20.GL_ONE /* GL_SRC_ALPHA */,
    ...
    /**
     * Decides whether we want to record the current frame, based on the target
     * frame rate
     * and an assumed 60fps refresh rate.
     * <p>
     * We could be smarter here, and not drop a frame if the system dropped one
     * inadvertently (i.e. we missed a vsync by being too slow).
     */
    ...
    /**
     * Handles pausing of the game Activity. This is called by the View (via
     * queueEvent) at
     * pause time. It tells GameState to save its state.
     * 
     * @param syncObj
     *            Object to notify when we have finished saving state.
     */
    public void onViewPause(ConditionVariable syncObj) {
        /*
         * We don't explicitly pause the game action, because the main game loop
         * is being driven
         * by the framework's calls to our onDrawFrame() callback. If we were
         * driving the updates
    ...
    /**
     * Updates state after the player touches the screen. Call through
     * queueEvent().
     */
    public void touchEvent(float x, float y) {
        /*
         * We chiefly care about the 'x' value, which is used to set the paddle
         * position. We
         * might want to limit based on the 'y' value because it's a little
         * weird to be
         * controlling the paddle from the top half of the screen, but there's
         * no need to
         * do so.
         * 
         * We need to re-scale x,y from window coordinates to arena coordinates.
         * The viewport
         * may not fill the entire device window, so it's possible to get values
         * that are
         * outside the arena range.
         * 
         * If we were directly implementing other on-screen controls, we'd need
         * to check for
         * touches here.
         */

        float arenaX = (x - mViewportXoff)
                * (GameState.ARENA_WIDTH / (float) mViewportWidth);
        float arenaY = (y - mViewportYoff)
                * (GameState.ARENA_HEIGHT / (float) mViewportHeight);
